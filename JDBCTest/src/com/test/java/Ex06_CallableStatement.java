...
public class Ex06_CallableStatement {
	...
	private static void m6() {
		...
		try {
			...
			stat = conn.prepareCall(sql); //SQL문을 보내기 위한 개체 생성과 동시에 sql을 넣는다.
			
			stat.setString(1, buseo);
			stat.registerOutParameter(2,  OracleTypes.CURSOR);
			
			stat.executeQuery(); //날리기
	...
	private static void m5() {
		...
		try {
			...
			rs = (ResultSet)stat.getObject(2); //이름은 getObject지만 반환은 ResultSet으로 변환해준다.
	...
	private static void m2() {
		...
		try {
			...
			//?에 값을 안 넣으면 뜨는 에러 > 인덱스에서 누락된 IN 또는 OUT 매개변수:: 1
			...
			int result = stat.executeUpdate(); //호출
			
			System.out.println(result); //성공 결과
