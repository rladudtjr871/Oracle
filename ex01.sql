...
alter user hr account lock; --계정 잠김
...
오라클
- 데이터 베이스(Database): 데이터 집합(=저장소) > DB
...
오라클(100) = ANSI-SQL(5~70) + PL/SQL(20~30) + 설계, 기타(10)
...
--파란색: 키워드 > SQL의 키워드(명령어)는 대소문자를 구별하지 않는다.
