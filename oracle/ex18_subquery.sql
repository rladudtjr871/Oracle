...
/*
    ...
    - 하나의 문장 안(SELECT, INSERT, UPDATE, DELETE)에 또 다른 문장(SELECT)이 들어있는 쿼리
    - 하나의 SELECT문 안에 또 다른 SELECT문이 들어있는 쿼리
    - SELECT > SELECT
    - INSERT > SELECT
    - UPDATE > SELECT
    - DELETE > SELECT
    - 삽입 위치 > SELECT절, FROM절, WHERE절, GROUP BY절, HAVING절, ODER BY절..
...
select name from tblCountry where population = 120660; -- A2 > 값의 업데이트에 취약 불편

--집계함수는 where절에서 쓸 수 없다 > 개개인의 값  전체의 값
...
-- 서브쿼리 삽입 위치
-- 1. 조건절 > 비교값으로 사용
--      *a. 반환값이 1행 1열 > 단일 값 반환 > 상수 취급 > 값 1개로 사용
--      *b. 반환값이 N행 1열 > 다중 값 반환 > 값 N개로 사용 > 열거형 비교 > in 사용
--      *c. 반환값이 1행 N열 > 다중 값 반환 > 그룹 비교 > N컬럼 : N컬럼 비교
--       d. 반환값이 N행 N열 > 다중 값 반환 > 2 + 3 >  in + 그룹 비교



-- 급여가 250만원 이상 받는 직원이 근무하는 부서의 직원 명단을 가져오시오.
...
-- 2. 컬럼리스트에서 사용
--      - 반드시 결과값이 1행1열이어야 한다.(****************) > 스칼라 쿼리
--      - 다른 컬럼과 관계있는 값을 반환해야 한다.(하나의 레코드의 모든 컬럼은 서로 연관이 있어야한다.)
--      1. 정적쿼리 > 모든 행에 동일한 값을 적용 > 사용 빈도 적음
...
-- 서브쿼리 > tblInsa
select name, buseo, jikwi, basicpay from (select name, buseo, jikwi from tblInsa); -- x > 서브쿼리에 basicpay가 없다.

-- from 서브쿼리가 별칭 사용 > 바깥쪽 메인쿼리는 별칭을 원래 컬럼명으로 인식
select name, buseo, jikwi/*position*/ from (select name, buseo, jikwi as position from tblInsa); -- x > jikwi의 이름을 바꿈

-- "length(name)" 의미? > 연산결과는 꼭 Alias를 붙여줘야한다.
...
select 
    name, height, couple,
    (select height from tblWoman where tblMan.couple = name) as height2 /*height*/
from tblMan
    order by height2 /*height*/ asc; -- 문제 발생!!! > height가 남자키? 여자키?
