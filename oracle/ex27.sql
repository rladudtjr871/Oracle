...
-- 일련번호가 필요한 업무 > 순차적으로 증가하는 숫자가 필요한 업무
-- 오라클 전용
...
-- 요구사항] 3월1일 ~ 3월 18일까지 날짜와 함께 근태정보를 가져오시오.
...
-- ** 기억!! > date자료형으로 원하는 기간의 레코드를 얻어오는 방법
...
select 
    v.regdate,
    t.state,
    to_char(v.regdate, 'd'),    --일주일중 몇번쨰 날인지
    case
...
insert into tblScore values (1, 100, 1, 1);--아직 못 넣음 오류때문에
