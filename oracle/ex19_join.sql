...
/*
    ...
    - 테이블을 다시 수정해야 고쳐지는 것들
    1. 테이블에 기본키가 없는 상태 > 데이터 조작 곤란
    2. null이 많은 상태의 테이블 > 공간 낭비
    3. 데이터가 중복되는 상태 > 공간 낭비 + 데이터 관리 곤란
    4. 하나의 속성값이 원자값이 아닌 상태 > 더 이상 쪼개지지 않는 값을 넣어야 한다. (취미: 독서,영화) -> (취미1: 독서 취미2: 영화)
...
-- 테스트, 25, 달리기,헬스,게임, 독서, 낮잠, 맛집, 여행, 코딩
-- 홍길북, 20, 독서, null, null, null, null, null, null, null
-- 홍길박, 20, 독서, null, null, null, null, null, null, null
-- 홍걸동, 20, 독서, null, null, null, null, null, null, null
-- 박길동, 20, 독서, null, null, null, null, null, null, null
-- 김밀동, 20, 독서, null, null, null, null, null, null, null
-- 최방동, 20, 독서, null, null, null, null, null, null, null
-- 정길묵, 20, 독서, null, null, null, null, null, null, null
-- 이길상, 20, 독서, null, null, null, null, null, null, null
-- 박무상, 20, 독서, null, null, null, null, null, null, null
...
select * from tblStaff where instr(project, '회원 응대') > 0; -- 이 방법은 예를 들면 '고객 회원 응대 처리'라는 업무까지 찾아버림;;
...
-- 급여 지급 > 담당자 > '홍길동' 급여 확인
-- 문제점!! > 같은 데이터를 2번 이상 저장 > 데이터 설계 문제!!!
...
-- A.3 신규 프로젝트 추가 > 문제발생!!! > 유령직원??
--ORA-02291: integrity constraint (HR.SYS_C007163) violated - parent key not found
...
-- B.2 '아무개 퇴사' > 인수 인계(위임)
update tblProject set staff_seq = 3 where staff_seq = 2; -- 가독성ㅠ

update tblProject set staff_seq = (select seq from tblStaff where name = '하하하')
    where staff_seq = (select seq from tblStaff where name = '아무개'); --가독성 good
...
-- 단순 조인 > 테이블 행 X 테이블 행 > 유효O + 유효X > 더미 테이블
...
select 
    count(distinct v.name) as "대여기록 있는 개수"
from tblVideo v
    inner join tblRent r
        on v.seq = r.video;


-- 대여 여부와 상관없이 모든 비디오와 그 대여 내역을 가져오시오.
select
    *
from tblVideo v
...
select
    m.name,
    count(rentdate) --***** 복습!!! > count(*)을 하면 대여기록이 없는 사람도 1로 나오는 이유에 대하여!!!
from tblMember m
    left outer join tblRent r
        on m.seq = r.member 
            group by m.name
...
from employees a                --매니저
    inner join employees b      --직원
        on a.employee_id = b.manager_id;  --a의 직원 아이디가 b의 매니저 아이디와 같으면
...
select
    *
from tblCustomer c
    full outer join tblSales s --left outer join 과 동일한 결과
        on c.seq = s.cseq;
