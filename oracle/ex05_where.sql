...
select name from tblCountry where continent = 'AS'; --where 조건을 만족하는 레코드의 컬럼만 가져온다.
...
-- 4. 급여(basicpay) + 수당(sudanag) 한 금액이 200만원 이상 받는 직원을 가져오시오. > 26명
select * from tblInsa
    where (sudang + basicpay) >= 2000000;
...
select * from tblInsa where ibsadate >= '2010-01-01' and ibsadate <= '2010-12-31'; --2010년도 입샇한 직원 명단
select * from tblInsa where ibsadate between '2010-01-01' and '2010-12-31'; --2010년도 입샇한 직원 명단
...
--'김'씨 > 김OO
select name from tblInsa where name like '김__'; -- _개수만큼 글자수 > 글자수가 상관 있다.
select name from tblInsa where name like '__수';
select name from tblInsa where name like '_길_';

select name from tblInsa where name like '김%'; --김 뒤로 오는 글자수는 상관이 없다. 
...
select * from employees where first_name like '%%a'; -- % 개수는 의미없다

select * from employees where first_name like '%a%'; --a로 시작하거나 끝나거나 중간에 들어가거나

select * from employees where first_name like 'S%a%'; --S로 시작 a가 중간 또는 끝에 들어가는 사람
...
select * from tblInsa where ssn like '%-2%'; -- 여기서 -가 아주 중요하다. 안 그러면 2가 들어가는 남자도 나온다.

select * from 도서정보 where 제목 like '%SQL%'; --예시
...
select * from  tblCountry where population is not null; -- o > *** 더 많이 사용(가독성)
...
-- 도서관 > 대여 테이블(속성, 속성,... 대여날짜, 반납 날짜)
-- 아직 반납을 안한 사람은?
select * from 도서대여 where 반납날짜 is null; --예시

-- 반납이 완료된 대여 기록?
select * from 도서대여 where 반납날짜 is not null; --예시
