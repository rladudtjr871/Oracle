...
select count(name) from tblCountry where continent = 'AS';  -- 아시아에 속한 나라 수?

select count(*) from tblCountry;
select count(name, capital, population, continent, area) from tblCountry; --위랑 같은 뜻이지만 이 문장은 에러
...
select population from tblCountry; -- null 값이 있어서 13개
...
select 
    ...
    count(*) - count(completedate) as "안할 일의 개수"
from tblTodo;


-- tblInsa 총 직원 명수? 연락처 있는 직원 수? 연락처 없는 직원 수?
select
    count(*) as "총 직원 수",
    count(tel) as "연락처 있는 직원 수",
...
-- 회사 성과급 지급 > 성과급 출처 > 팀 1의 공로
-- 1. 균등 지급 : 총 지급액 / 모든 팀원 수 = sum() / count(*)
-- 2. 차등 지급 : 총 지급액 / 참여 인원 수 = sum() / count(참여인원) = avg()
...
--   - 컬럼 리스트에 집계 함수와 컬럼은 동시에 사용할 수 없다. > 성질이 다르다.
...
select * from tblInsa where basicpay >= avg(pasicpay);
