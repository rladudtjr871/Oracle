...
select name  as "[남자이름]", capital as "[수도명]", population as "[인구수]", area as "[면적]", continent as "[대륙]"
    from tblCountry;
...
select (first_name || last_name) as "이름", salary as "급여", phone_number as "연락처"
    from employees
...
select (first_name || last_name) as "이름", phone_number as "연락처", (email || '@gmail.com') as "이메일", department_id as "부서ID"
    from employees
...
select name as "이름", jikwi as "직위", tel as "전화번호"
    from tblInsa
...
select *
    from tblInsa
    where ssn like '__07__-_______' or ssn like  '__08__-_______' or ssn like '__09__-_______';
...
select *
    from tblInsa
    where city in ('서울', '인천', '경기') and ibsadate between '2008-01-01' and '2010-01-01';
