...
/*
    ...
    - 특정 컬럼의 중복 배제(x) > 레코드의 중복 배제(o) > 레코드(컬럼,컬럼,컬럼)
...
-- 중복값이 존재하지 않을 때 distinct > 아무일도 안 일어난다.
...
-- *** 공용 데이터와 개별 데이터는 메모리 한 곳에 넣을 수 없다. > 공용(static), 개별(stack) / 자바

select distinct continent, /*distinct*/ name from tblCountry; -- 컬럼리스트에 distinct는 한번만 쓸 수 있다.
...
select 
    ...
    case
        ...
        --else '기타'
        --else continent -- 원본을 돌려준다는 뜻
        --else name > 사용 금지!!! ** 하나의 속성(컬럼) 안에는 같은 도메은(성격,표현)을가지는 값이 들어간다.(*****)
        --else population > 자료형이 달라서 에러
        -- *** 모든 반환값이 동일한 성격 + 동일한 자료형을 가져야한다.
