...
-- tblTodo 한 일의 개수? 안 한 일의 개수? > 그룹 기준? 일의 완료 유무?
...
select                                      --4. 평균급여가 150만원이 넘는 그룹
    buseo,
    round(avg(basicpay))
from tblInsa                                --1. 60명의 데이터를 가져온다.
    group by buseo                          --2. 60명을 대상으로 그룹 형성.
        having avg(basicpay) >= 1500000;    --3. 각 그룹을 대사으로 조건 > 조건에 안 맞는 그룹을 탈락!!!
...
from tblInsa                                    --1. 60명 데이터를 가져온다
    where basicpay >= 1500000                   --2. 27명만 남긴다. (개인탈락)
        group by buseo                          --3. 27명을 가지고 그룹형성
            having avg(basicpay) >= 2200000     --4. 4부서만 남긴다. (부서탈락 > 2부서 버려짐)
                order by 2;                     --6. 정렬
                


-- 서울 직원 > 남/여 그룹 > 그룹별 평균 급여가 155만원 이상 > 각 그룹의 인워수?
select                                           --5.
    substr(ssn, 8, 1),
    count(*)
from tblInsa                                     --1.
...
-- tblCountry
-- 대륙별로 최대 인구수, 최소 인구수, 평균 인구수를 가져오시오.
select
    continent,
    max(population),
    min(population),
