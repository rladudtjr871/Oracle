...
/*
    ...
    데이터베이스
    - 테이블 내에 레코드 순서는 우리가 원하는 정렬 상태가 아니다. > 정렬 개념 X
    ...
    1. Primary key
    2. Unique
    - *** 일반 테이블에서 PK컬럼을 검색하는 속도 >> 일반 테이블에서 PK가 아닌 컬럼을 검색하는 속도
...
select count(*) from tblIndex; --백만건 정도~ 7,864,320
...
-- 인덱스 없이 검색 > 경과 시간: 00:00:02.376 > 2초
...
-- 인덱스 검색 > 경과 시간: 00:00:00.097 > 0.1초
select distinct name from tblIndex where name = '홍길동';
...
-- : 색인의 값이 중복이 불가능하다.
-- : PK, UNIQUE
create unique index idxName on tblIndex(name); -- 동명이인이 없다는 가정
...
select count(*) from tblIndex where buseo = '기획부'; -- 인덱스 효과 O > buseo는 인덱스가 적용된 상태

select count(*) from tblIndex where buseo = '기획부' and name = '홍길동'; -- 인덱스 효과 X > buseo는 인덱스 적용이지만 name은 아니다.
...
select count(*) from tblIndex where buseo = '기획부' and name = '홍길동'; -- 인덱스 효과 O > 복합 인덱스를 적용하면 빨라진다.
select count(*) from tblIndex where name = '홍길동' and buseo = '기획부'; -- 컬럼 순서가 바뀌면 안되는 버전도 있음. > 현재 버전 얘기는 아님
...
select count(*) from tblIndex where substr(ssn, 8, 1) = '1'; -- 4초 > 2초 > 여전히 오래걸림.. > 가공자체를 인덱스화 > 0.7초
