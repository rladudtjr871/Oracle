...
select * from tblUser where id = 'test' and pw = pkgCrypto.encrypt('1234', 'java'); -- 1234는 사용자 입력값
...
-- 2. MD5
-- 3. SH1
SELECT RAWTOHEX(DBMS_CRYPTO.HASH(TO_CLOB(TO_CHAR('test')), 1)),
       RAWTOHEX(DBMS_CRYPTO.HASH(TO_CLOB(TO_CHAR('test')), 2)),
       RAWTOHEX(DBMS_CRYPTO.HASH(TO_CLOB(TO_CHAR('test')), 3))from dual;
