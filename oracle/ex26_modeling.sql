...
/*
   ...
   - 이때 DBMS의 종류를 결정하지 않는다. > 추상화
   - 개념 + 논리 + 물리
   - 개념 > 아주 간단히 표현한 설계도 > 테이블 이름과 관계 정도만 기술
   - 논리 > 관계형 데이터베이스의 기본 설정 > 속성 정의 + 도메인 정의 + 키(PK, FK).. > 여기부터 제대로된 ERD
    ...
    3. Relationship
        ...
        - 관계의 종류(패턴)
            ...
            a. 1:1
                - 1개의 A는 1개의 B와 관계 > -|----|-
            b. 1:0
                - 1개의 A는 0개의 B와 관계 > 관계(x) -|----O- , -|----O|-
            c. 1:N
                - 1개의 A는 1개 이상의 B와 관계 > -|----K-|-
            d. 1:N
                - 1개의 A는 0개 이상의 B와 관계 > -|----OK-|-
                - b + c
...
----------------------------- 물리 다이어그램 > 물리 ERD
    ...
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하며
      , 데이터 종소겅을 최소화하기 위해 ERD를 수정하는 작업
    ...
    1. 삽입 이상, Insertion Abnormaly
        - 특정 테이블에 데이터를 삽입할 때 데이터까지 같이 넣어야 하는 상황
        
    2. 갱신 이상
        - 동일한 데이터가 2개 이상의 테이블에 동시 저장되어있는데 
            > 그 중 1개는 수정했지만 다른 1개는 수정하지 못 했을 때 상황 > 두 데이터 간의 이상 오류 발생
    ...
    - 정규화는 '부분 함수 종속'이나 '이행 함수 종속'을 모두 없애고,
      모든 컬럼 관계를 '완전 함수 종속'으로 만든느 작업이다.
    ...
    - 주로 복합키를 가지는 테이블에서 발견되며, 
      ...
      복합키 일부에만 종속되는 현상이 발견되면 > 현상 제거
        
    완전 함수 종속    
    - 기본키가 아닌 컬럼이 기본키에게 종속적인 상태
